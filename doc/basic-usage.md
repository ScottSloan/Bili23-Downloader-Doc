...
程序启动后分为四大核心功能区，你可以通过侧边栏/顶部的标签导航进行切换：
- **解析页**：粘贴视频链接并获取可下载的栏目列表，是程序的默认首页。
- **下载页**：集中追踪下载进度，管理正在下载和已完成的任务。
- **收藏页**：查看登录账号的收藏夹、订阅合集、追番追剧、稍后再看、历史记录等内容。
...
如果是面对包含成百上千个视频的大型合集，可以使用以下高级功能辅助选择：
...
::: details 🖱️ 进阶技巧：点击表头进行升降序排列
在解析列表的顶部表头区域，**直接点击**对应的列标题（如“序号”、“时长”、“发布时间”），即可快速按该属性对内容进行排序。再次点击同一列标题，即可在 **升序 / 降序**（正序/倒序）之间灵活切换。

<img src="./pic/sort_by_column.png" alt="按列排序示意图">
:::
...
## 4. 浏览账号收藏与记录
<img src="./pic/favorites_page.png" alt="收藏页示意图">

切换至 **[收藏页]**，在确保账号已登录的情况下，你可以直接在这里快捷读取和访问你的个人内容资产，省去切回浏览器找链接的麻烦：
- **个人资产**：支持加载自己创建的“收藏夹”、“稍后再看”以及近期的“历史记录”。
- **追番列表**：一键同步你的“追番追剧”列表与“订阅合集”。

::: tip 🚀 联动解析：告别复制粘贴
当你在本页面找到想要下载的内容时，只需点选对应的卡片，程序会自动将底层链接提取并直接跳转至 **[解析页]** 进行深度解析。这种“所见即所得”的联动设计能让你获得极其流畅的下载体验。
:::
